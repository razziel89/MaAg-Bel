...
#undef OBAtomIncrement
  ...
  //! beforehand.
  bool OBMol::BondMolecule(OBMol &mol, int i1, int i2, int m1, int m2){
    OBMol bondmol;;
    if (this == &mol){
      bondmol = OBMol(mol);
    }
    ...
    bondmol.Translate(-bond_center2);
    bondmol.Rotate(tempvec, angle);
    bondmol.Translate(bond_center1);
  ...
  void OBMol::AppendMolecule(OBMol &src)
  {
    vector<OBAtom*>::iterator i;
    vector<OBBond*>::iterator j;
    OBAtom *atom, *a;
  ...
  bool OBMol::CleaveOff(int i1, int i2){
    std::vector<OBAtom*> cleave_atoms;
    OBAtom *a1,*a2;
    a1 = GetAtom(i1);
    a2 = GetAtom(i2);
    FindChildren(cleave_atoms, a1 , a2);
  ...
  void OBMol::PartMolecule(OBMol &dest,  const double direction[3], const double point[3])
  {
    vector<OBAtom*>::iterator i;
    OBAtom *atom, *a;
    unsigned int at_count=1;
  ...
  void OBMol::Rotate(std::vector<OBAtom*> &atom_vec, const matrix3x3 m)
  {
    OBAtom *atom;
    vector<OBAtom*>::iterator i;
    vector3 tempvec;
  ...
  void OBMol::Rotate(const int main_axis_nr, const double angle)
  {
    Rotate(_vatom, main_axis_nr, angle);
  }

  //! \brief Rotate a subset of atoms of a molecule one of its three main axes by an angle in degrees.
  void OBMol::Rotate(std::vector<OBAtom*> &atom_vec, const int main_axis_nr, const double angle)
  {
  ...
  void OBMol::Translate(const double vector[3])
  {
  ...
  void OBMol::Translate(std::vector<OBAtom*> &atom_vec, const double vec[3])
  {
  ...
  void OBMol::Align(const double p[3], const double v1[3], const double v2[3])
  {
  ...
  void OBMol::Align(const vector3 p, const vector3 v1, const vector3 v2)
  {

    Align(_vatom, p, v1, v2);
  }
...
#undef streched
    ...
    if (children.size() == NumAtoms()-1){cerr << "all atoms connected\n";return false;};
  ...
  void OBMol::Mirror(const double normal[3], const double point[3], bool center_it)
  {
  ...
  //! If center_it==true, the molecule's center is temporarily moved to the origin
  //! before applying the transformation and moved to its original position afterwards.
  void OBMol::Mirror(const vector3 normal, const vector3 point, bool center_it){
    Mirror(_vatom, normal, point, center_it);
  }
  ...
  //! If center_it==true, the molecule's center is temporarily moved to the origin
  //! before applying the transformation and moved to its original position afterwards.
  void OBMol::Mirror(std::vector<OBAtom*> &atom_vec, const vector3 normal, const vector3 point, bool center_it)
  {
...
#undef  OBBondIncrement
  ...
  */
  ...
  void OBMol::AddConformer(double* f, bool deepcopy)
  {
      if (deepcopy){
          double* new_f = new double[3*NumAtoms()];
          memcpy((char*)new_f,(char*)f,sizeof(double)*3*NumAtoms());
          f = new_f;
      }
      _vconf.push_back(f);
  }
  ...
  void OBMol::DeleteConformers(int start_idx, int end_idx)
  {
    ...
    _vconf.erase((_vconf.begin()+start_idx), (_vconf.begin()+end_idx+1));
  }
